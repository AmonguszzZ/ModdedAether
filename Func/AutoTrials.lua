...
local task = task -- ensures task library is available
...
local function executeTrialScript(matchedTrialName)
    ...
    for attempt = 1, maxRetries do
        print(string.format("[AutoTrials] Loading execution script for '%s' from: %s (Attempt %d/%d)", targetKey, scriptUrl, attempt, maxRetries))
        ...
        if success then
            ...
            warn(string.format("[AutoTrials] Error executing script for '%s' (Attempt %d): %s", targetKey, attempt, tostring(err)))
            if attempt < maxRetries then
                print(string.format("[AutoTrials] Retrying in %d seconds...", retryDelay))
                task.wait(retryDelay)
            else
                warn(string.format("[AutoTrials] All %d attempts failed for '%s'.", maxRetries, targetKey))
            end
        end
    end
end
